...
public class main_navigation_admin extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_main_navigation_admin, container, false);

        id = view.findViewById(R.id.id);
        password = view.findViewById(R.id.adminpassword);
        login = view.findViewById(R.id.tpologin);

        login.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                client = new OkHttpClient();
                String url = "http://" + AppConfig.SERVER_IP + ":4000/adminlogin";
                String id_data = id.getText().toString();
                String password_data = password.getText().toString();

                JSONObject jsonBody = new JSONObject();
                try {
                    jsonBody.put("id", id_data);
                    ...
                    public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                        ...
                        if (response.isSuccessful()) {
                            ...
                            if(item.equals("Authorized")){
                                ...
                                Intent i = new Intent(requireContext(), NavAdminActivity.class);
                                startActivity(i);
                            }
                            else {
